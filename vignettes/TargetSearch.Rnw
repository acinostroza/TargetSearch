...
the function to see other samples. In the figure~\ref{fig:chmrim}
a example is shown. The gray area indicates the search window of each
marker.

\begin{figure}[htp]
\centering
<<checkRimLim, fig=true, width=7, height=7>>=
checkRimLim(samples, rimLimits)
@
\caption{Example of the RI markers $m/z$ traces in one sample}\label{fig:chmrim}
\end{figure}
...
sample and retention time limits objects, the intensity threshhold (\Rfunarg{IntThreshold = 10}),
...
<<RIcorrection>>=
RImatrix <- RIcorrect(samples, rimLimits,
