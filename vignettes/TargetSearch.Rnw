...
\Rfunction{baselineCorrection()}, the so-called ``classic'' method because it has been
present since de beginning, and the algorithm is based on the work of
Chang et al. \cite{chang07}. The second method, ``quantiles'', was introduced
in \Biocpkg{TargetSearch} 1.42.0 and the baseline is estimated by computing a quantile in a sliding
window. This is implemented in \Rfunction{baselineCorrectionQuant()}.
...
To baseline correct the chromatograms, we would call the function
\Rfunction{ncdf4Convert()}, similarly as the section above but with
extra parameters as the example shown below.
...
to \Rfunction{baselineCorrectionQuant()}. Similarly, if the method is ``classic'',
